...
# --- Layout Split (Mesop Style) ---
st.sidebar.markdown("## 🖥️ Layout Settings")
show_code = st.sidebar.toggle("Show Developer Code Palette", value=True)

if show_code:
    main_col, code_col = st.columns([1.4, 1])
else:
    main_col, code_col = st.container(), None
        ...
        with tab3:
            ...
            with l_col2:
                ...
                if entities:
                    ...
                    st.info("No specific named entities found.")
...
if code_col:
    with code_col:
        st.markdown("#### 💻 Developer Palette")
        if data:
            st.markdown("##### Raw Analysis Payload")
            st.caption("Standardized JSON Schema consumed by downstream microservices.")
            st.json(data)
        else:
            st.info("Run an analysis to inspect the generated JSON schema.")
            
        st.markdown("##### cURL API Integration")
        st.code("""
curl -X POST "http://localhost:8000/api/analyze" \\
     -H "Content-Type: application/json" \\
     -d '{"text": "URGENT: Broken payment!"}'
        """, language="bash")
...
result = analyzer.analyze("URGENT: Broken payment!")
